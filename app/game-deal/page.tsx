...
export default function GameDealsPage() {
  ...
  const fetchDeals = useCallback(
    async (currentSearch: string, currentStore: string) => {
      setLoading(true);
      setError(null);
      try {
      ...
      <div className="mx-auto max-w-7xl">
        <Breadcrumbs
          items={[
            ...
            { label: "Game Deals", href: "/game-deal", active: true },
          ]}
          className="mb-8"
        />
        <div className="mb-8">
          ...
          <p className="mt-2 text-muted-foreground google-sans">
            Find the best discounts on games across various digital stores using
            CheapShark API.
          </p>
        </div>
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-4">
          ...
          <Button
            onClick={() => fetchDeals(searchTerm, selectedStore)}
            disabled={loading}
            className="h-11 shadow-lg shadow-primary/20 font-local-inter"
          >
            {loading ? (
            ...
            {deals.map((deal) => (
              ...
              >
                <div className="relative aspect-video overflow-hidden bg-muted">
                  <Image
                    src={
                      imageError[deal.dealID]
                        ? "https://placehold.co/720x480?text=Image+Not+Found"
                        : deal.thumb ||
                          "https://placehold.co/720x480?text=No+Image"
                    }
                    alt={deal.title}
                    fill
                    loading="eager"
                ...
                <CardContent className="p-4 pt-0">
                  <div className="flex items-end justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold font-local-inter">
                        Deal Rating
                      </p>
                      <div className="flex items-center gap-1 justify-end">
                        <span className="text-lg font-local-jetbrains-mono font-bold text-primary">
                          {deal.dealRating}
                        </span>
                      </div>
                    </div>
        ...
        {!loading && deals.length === 0 && (
          <div className="flex h-96 flex-col items-center justify-center text-center rounded-2xl border border-dashed border-border bg-card/20 backdrop-blur-sm">
            ...
            <p className="text-muted-foreground mb-6">
              We couldn&apos;t find any active deals matching your criteria.
            </p>
            <Button
              variant="outline"
              onClick={() => {
