...
export default function PostsPage() {
  ...
  return (
    <div className="min-h-screen px-6 py-12 lg:px-8 posts_page img_grad_pm">
      <div className="mx-auto max-w-4xl">
        <div className="mb-12">
          <h1 className="mb-4 text-4xl font-bold mozilla-headline">Posts & Updates</h1>
        ...
        {posts.length === 0 ? (
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            <p className="text-muted-foreground">
              No posts yet. Create a <code className="rounded bg-muted px-2 py-1 font-mono text-sm">.md</code> or{" "}
              <code className="rounded bg-muted px-2 py-1 font-mono text-sm">.html</code> file in the{" "}
              <code className="rounded bg-muted px-2 py-1 font-mono text-sm">content/posts/</code> directory.
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            {posts.map((post, index) => {
              ...
              const backgroundStyle = post.firstImage
                ? {
                    backgroundImage: `linear-gradient(45deg, #00000000 0%, #0000003d 30%, #00000063 40%, #00000088 50%, #000000c5 65%, #000000 90%), url("${post.firstImage}")`,
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {post.description && (
                        <p className="mb-3 text-muted-foreground leading-relaxed">{post.description}</p>
                      )}
                      {post.date && (
