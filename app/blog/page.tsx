...
export default async function BlogPage() {
  ...
  const notionPosts = await getNotionContentByType("blog")
  ...
  const allPosts = [
    ...
    ...notionPosts.map(post => ({
      ...
      date: post.date,
      type: 'blog' as const,
      content: '', // Not needed for the list
      rawContent: '',
  ...
  const hasFinalPost = allPosts.some((post) => (post as any).final)
        ...
        {allPosts.length === 0 ? (
          <div className="rounded-xl border border-border bg-card p-12 text-center">
            <p className="text-muted-foreground">
              ...
              <code className="rounded bg-muted px-2 py-1 font-mono text-sm">content/blog/</code> directory or configure Notion.
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            {allPosts.map((post) => {
              const isFinal = (post as any).final;
