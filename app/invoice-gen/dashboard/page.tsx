import { getDashboardStats, getRecentInvoices } from "@/app/invoice-gen/actions/dashboard";
import { getSettings } from "@/app/invoice-gen/actions/settings";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { formatCurrency, formatDate, cn } from "@/lib/utils";
import { ReceiptText, Users, CreditCard, AlertCircle, ArrowRight, Plus, Settings } from "lucide-react";
...
export default async function DashboardPage() {
      ...
      <Card className="border-primary/20 bg-primary/5 backdrop-blur-md rounded-3xl overflow-hidden">
        <CardContent className="p-12 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-bold google-sans">Configuration Required</h2>
            <p className="text-muted-foreground max-w-md mx-auto">
              Please set up your business profile in settings to start generating invoices.
            </p>
          </div>
  ...
  const statCards = [
    { title: "Total Invoices", value: stats.totalInvoices, icon: ReceiptText, color: "text-blue-500", bg: "bg-blue-500/10" },
    { title: "Amount Paid", value: formatCurrency(stats.amountPaid, settings.currency), icon: CreditCard, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    { title: "Outstanding", value: formatCurrency(stats.outstanding, settings.currency), icon: Users, color: "text-orange-500", bg: "bg-orange-500/10" },
    { title: "Overdue", value: formatCurrency(stats.overdue, settings.currency), icon: AlertCircle, color: "text-rose-500", bg: "bg-rose-500/10" },
  ];
        ...
        <Card className="bg-card/20 backdrop-blur-md border-border/40 rounded-3xl overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between p-6 border-b border-border/40">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Your most recently created invoices.</p>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border/40 bg-muted/5">
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Number</th>
                  ...
                  {recentInvoices.length > 0 ? (
                    recentInvoices.map((invoice) => (
                      <tr key={invoice.id} className="border-b border-border/40 hover:bg-muted/10 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-sm font-bold">{formatCurrency(invoice.totalAmount, settings.currency)}</td>
                        <td className="px-6 py-4">
