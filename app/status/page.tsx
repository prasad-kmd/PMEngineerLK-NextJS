...
export default async function PublicStatusPage() {
  ...
  const currentStatus =
    statusConfig[data.overall_status as keyof typeof statusConfig] ||
    statusConfig.major_outage;
      ...
      <div className="max-w-5xl w-full space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        <div className="flex flex-col items-center text-center space-y-4">
          <Link href="/" className="group flex items-center gap-2 mb-4">
            ...
            <span className="text-xl font-black amoriaregular tracking-tighter">
              PMEngineerLK
            </span>
          </Link>
          ...
          <p className="text-muted-foreground font-medium max-w-md">
            Real-time status updates for our engineering platform services and
            infrastructure.
          </p>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-6">
            <div
              className={`w-16 h-16 rounded-full ${currentStatus.color} flex items-center justify-center shadow-[0_0_20px_rgba(var(--status-color),0.4)] relative`}
            >
              <div
                className={`absolute inset-0 rounded-full ${currentStatus.color} animate-ping opacity-20`}
              />
              <ShieldCheck className="w-8 h-8 text-white" />
            </div>
            <div className="text-left">
              <h2
                className={`text-2xl md:text-3xl font-black amoriaregular tracking-tight ${currentStatus.text}`}
              >
                {currentStatus.label}
              </h2>
          ...
          <Badge
            variant="outline"
            className={`${currentStatus.border} ${currentStatus.text} px-4 py-1.5 rounded-full font-mono text-xs font-black`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.services.map((service: unknown) => (
            <div
              key={(service as { name: string }).name}
              className="group rounded-3xl border border-border/50 bg-card/40 p-6 flex flex-col gap-4 hover:border-primary/30 transition-all hover:shadow-md backdrop-blur-sm"
            >
              <div className="flex items-start justify-between">
                <div className="p-3 rounded-2xl bg-muted/50 text-muted-foreground group-hover:text-primary transition-colors border border-border/40">
                  {serviceIcons[
                    (service as { name: string })
                      .name as keyof typeof serviceIcons
                  ] || <Database className="w-5 h-5" />}
                </div>
                <StatusIndicator
                  status={(service as { status: string }).status}
                  withLabel
                  size="md"
                />
              </div>
