...
import { Zap, Info, Search } from "lucide-react";
...
export default function VoltageDividerDesigner() {
  ...
  const results = useMemo(() => {
    ...
    const seriesValues =
      series === "E12" ? getStandardValues(E12) : getStandardValues(E24);
    ...
    if (idx1 < seriesValues.length - 1)
      r1Candidates.push(seriesValues[idx1 + 1]);

    const idx2 = seriesValues.indexOf(r2Candidates[0]);
    if (idx2 > 0) r2Candidates.push(seriesValues[idx2 - 1]);
    if (idx2 < seriesValues.length - 1)
      r2Candidates.push(seriesValues[idx2 + 1]);
    ...
    return {
      r1:
        bestR1 >= 1000
          ? (bestR1 / 1000).toFixed(2) + " kΩ"
          : bestR1.toFixed(1) + " Ω",
      r2:
        bestR2 >= 1000
          ? (bestR2 / 1000).toFixed(2) + " kΩ"
          : bestR2.toFixed(1) + " Ω",
      ...
      <div className="mx-auto max-w-4xl">
        <Breadcrumbs
          items={[
            ...
            {
              label: "Voltage Divider Designer",
              href: "/tools/voltage-divider-designer",
              active: true,
            },
          ]}
          className="mb-8"
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">
            Find the best standard resistor values (E12/E24) for your desired
            output voltage.
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-200 text-xs">
              ...
              <p>
                This tool searches through standard resistor series to find the
                pair that most closely matches your target output voltage while
                staying near your target current.
              </p>
            </div>
          </div>
              ...
              <CardContent className="flex-1 flex flex-col justify-center space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center p-3 rounded-lg bg-background/30 border border-border/30">
                    ...
                    <span
                      className={`font-mono font-bold ${parseFloat(results?.error || "0") > 1 ? "text-red-400" : "text-green-400"}`}
                    >
                ...
                <div className="p-4 rounded-lg border border-border/50 bg-background/30 mt-auto">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Ensure your resistors can handle the power: P = V²/R. For a{" "}
                    {vin}V input, total power is{" "}
                    {(
                      parseFloat(vin) ** 2 /
                      (parseFloat(results?.r1 || "0") +
                        parseFloat(results?.r2 || "0") || 1)
                    ).toFixed(4)}
                    W.
                  </p>
                </div>
