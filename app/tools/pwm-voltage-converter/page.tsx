...
export default function PWMVoltageConverter() {
  const [peakVoltage, setPeakVoltage] = usePersistentState<string>(
    "pwm-vpeak",
    "5",
  );
  const [dutyCycle, setDutyCycle] = usePersistentState<number>("pwm-duty", 50);
  const [targetVoltage, setTargetVoltage] = usePersistentState<string>(
    "pwm-vtarget",
    "2.5",
  );
      ...
      <div className="mx-auto max-w-4xl">
        <Breadcrumbs
          items={[
            ...
            {
              label: "PWM to Voltage Converter",
              href: "/tools/pwm-voltage-converter",
              active: true,
            },
          ]}
          className="mb-8"
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">
            Convert PWM duty cycle to average DC voltage and vice-versa. Ideal
            for microcontroller signal processing.
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="space-y-6">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    value={peakVoltage}
                    onChange={(e) => setPeakVoltage(e.target.value)}
                  />
                </div>
                ...
                <div className="pt-4 border-t border-border/50">
                  <div className="space-y-2">
                    <Label htmlFor="target-v">
                      Find Duty Cycle for Target Voltage (V)
                    </Label>
                    <Input
                      id="target-v"
                      type="number"
            ...
            <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-200 text-xs">
              ...
              <p>
                Pulse Width Modulation (PWM) creates an average DC voltage by
                rapidly switching a signal between 0V and peak voltage. The
                average voltage is directly proportional to the duty cycle.
              </p>
            </div>
          </div>
            ...
            <Card className="border-primary/20 bg-primary/5 h-full flex flex-col">
              ...
              <CardContent className="flex-1 flex flex-col justify-center space-y-8">
                ...
                <div className="p-6 rounded-xl bg-background/50 border border-border/50 text-center">
                  <div className="text-xs uppercase text-muted-foreground font-bold mb-2">
                    Required Duty for {targetVoltage}V
                  </div>
                  <div className="text-3xl font-bold text-muted-foreground tabular-nums">
                    {parseFloat(results.requiredDuty) > 100
                      ? "Impossible (>100%)"
                      : results.requiredDuty + "%"}
                  </div>
                </div>
                ...
                <div className="p-4 rounded-lg border border-border/50 bg-background/30 mt-auto">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    If your MCU has an 8-bit PWM, multiply the percentage by
                    2.55 to get the register value (0-255). For 10-bit, multiply
                    by 10.23 (0-1023).
                  </p>
                </div>
              </CardContent>
