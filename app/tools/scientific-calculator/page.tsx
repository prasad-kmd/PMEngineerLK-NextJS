...
export default function ScientificCalculator() {
  ...
  const calculate = (isAuto = false) => {
    ...
    try {
      ...
      if (resStr !== "Error") {
        ...
        if (!isAuto) {
          setHistory((prev) =>
            [expression + " = " + resStr, ...prev].slice(0, 5),
          );
        }
      } else if (!isAuto) {
        setResult("Error");
  ...
  const buttons = [
    ...
    { label: ")", value: ")" },
    {
      label: <RotateCcw className="w-4 h-4 text-orange-500" />,
      value: "clear",
      action: clear,
    ...
    { label: ".", value: "." },
    {
      label: <Delete className="w-4 h-4 text-red-500" />,
      value: "backspace",
      action: backspace,
      variant: "outline" as const,
    },
      ...
      <div className="mx-auto max-w-5xl w-full flex flex-col h-full">
        <Breadcrumbs
          items={[
            ...
            {
              label: "Precision Engineering Calculator",
            ...
            <Card className="shadow-sm border-primary/20 bg-primary/5 backdrop-blur-sm border-l-4 border-l-primary">
              <CardContent className="p-2 sm:p-3 h-full flex flex-col justify-between">
                <div className="flex justify-between items-center mb-1">
                  ...
                  {result && result !== "Error" && (
                    <div className="text-[8px] font-mono text-primary/70 px-1.5 py-0.5 bg-primary/10 rounded">
                      AUTOCALC
                    </div>
                  )}
                </div>
                <div className="flex-1 flex items-center justify-end overflow-hidden">
                  {result !== null && (
                    <span
                      className={`text-2xl sm:text-3xl font-bold truncate ${result === "Error" ? "text-destructive" : "text-primary"} animate-in zoom-in-95 duration-200`}
                    >
                      {result}
                    </span>
                  )}
                  {result === null && (
                    <span className="text-muted-foreground/20 text-2xl font-bold font-mono">
                      0.00
                    </span>
                  )}
                </div>
              ...
              <div className="grid grid-cols-4 gap-1.5 flex-1 min-h-0">
                {buttons.map((btn, idx) => (
                  <Button
                    ...
                    className={`h-full min-h-[40px] text-xs sm:text-sm font-medium transition-all active:scale-[0.95] ${
                      typeof btn.label === "string" && !isNaN(Number(btn.label))
                  ...
                  {history.length > 0 ? (
                    <div className="space-y-1.5 max-h-[120px] overflow-y-auto">
                      {history.map((item, i) => (
                        <div
                          key={i}
                          className="text-[11px] font-mono p-1.5 rounded bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer truncate"
