...
export default function Timer555Calculator() {
    ...
    const results = useMemo(() => {
        if (mode === "astable") {
            ...
            return {
                frequency: frequency >= 1000 ? (frequency / 1000).toFixed(2) + " kHz" : frequency.toFixed(2) + " Hz",
                period: period >= 1 ? period.toFixed(2) + " s" : (period * 1000).toFixed(2) + " ms",
            ...
            return {
                pulseWidth: pulseWidth >= 1 ? pulseWidth.toFixed(2) + " s" : (pulseWidth * 1000).toFixed(2) + " ms"
            }
        }
    }, [mode, r1, r2, c1, rm, cm])
            ...
            <div className="mx-auto max-w-4xl">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-muted-foreground mt-2">
                        Calculate component values for Astable and Monostable 555 timer circuits.
                    </p>
                </div>
                    ...
                    <div className="grid gap-8 lg:grid-cols-2">
                        <div className="space-y-6">
                                    ...
                                    {mode === "astable" ? (
                                        <>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Input id="r2" type="number" value={r2} onChange={(e) => setR2(e.target.value)} />
                                            </div>
                                            <div className="space-y-2">
                                                <Label htmlFor="c1">Capacitor C (µF)</Label>
                                                <Input id="c1" type="number" value={c1} onChange={(e) => setC1(e.target.value)} />
                                            </div>
                                        </>
                                    ) : (
                                        <>
                            ...
                            <div className="flex items-start gap-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[11px] text-blue-200">
                                ...
                                <div>
                                    <p className="font-semibold mb-1">Theoretical Formulas:</p>
                                    {mode === "astable" ? (
                                        <p>f = 1.44 / ((R1 + 2*R2) * C)<br />Duty Cycle = (R1 + R2) / (R1 + 2*R2)</p>
                                    ) : (
                                        <p>Pulse Width (T) = 1.1 * R * C</p>
                                    )}
                                </div>
                            </div>
                        </div>
                                    ...
                                    {mode === "astable" ? (
                                        <div className="space-y-6">
                                            <div className="text-center">
                                                <div className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-1">Frequency</div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="p-3 rounded-xl bg-background/50 border border-border/50">
                                                    <div className="text-[10px] uppercase text-muted-foreground font-bold mb-1">Duty Cycle</div>
                                                    <div className="text-lg font-semibold tabular-nums">{results?.dutyCycle || "--"}</div>
                                                </div>
                                                <div className="p-3 rounded-xl bg-background/50 border border-border/50">
                                                    <div className="text-[10px] uppercase text-muted-foreground font-bold mb-1">Period</div>
                                                    <div className="text-lg font-semibold tabular-nums">{results?.period || "--"}</div>
                                                </div>
                                                <div className="p-3 rounded-xl bg-background/50 border border-border/50">
                                                    <div className="text-[10px] uppercase text-muted-foreground font-bold mb-1">Time High (tH)</div>
                                                    <div className="text-lg font-semibold tabular-nums">{results?.th || "--"}</div>
                                                </div>
                                                <div className="p-3 rounded-xl bg-background/50 border border-border/50">
                                                    <div className="text-[10px] uppercase text-muted-foreground font-bold mb-1">Time Low (tL)</div>
                                                    <div className="text-lg font-semibold tabular-nums">{results?.tl || "--"}</div>
                                                </div>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="text-center py-12">
                                            <div className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-2">Pulse Width (T)</div>
                                            ...
                                            {mode === "astable" ? (
                                                Array.from({ length: 20 }).map((_, i) => (
                                                    <div key={i} className={cn(
                                                        "h-full w-2",
                                                        i % 4 < 2 ? "bg-blue-500/40 border-t-2 border-blue-500" : "bg-transparent border-b-2 border-blue-500"
                                                    )} />
                                                ))
                                            ) : (
