...
import { env } from "../../env";
import { getPlainText, getImageUrl } from "../utils";
...
<div class="notion-embed my-8 aspect-video rounded-3xl overflow-hidden border border-border/50 shadow-sm relative group">
  ...
  n2m.setCustomTransformer("toggle", async (block) => {
    ...
    try {
      ...
      // We use a custom tag here too to ensure it passes through everything safely
      return `\n\n<NOTION_TOGGLE_START title="${title.replace(
        /"/g,
        "&quot;",
      )}">\n\n${parent}\n\n</NOTION_TOGGLE_END>\n\n`;
    } catch (error) {
  ...
  n2m.setCustomTransformer("button", async (block) => {
    ...
    return `\n\n<NOTION_BUTTON label="${label.replace(
      /"/g,
      "&quot;",
    )}" url="${url}"></NOTION_BUTTON>\n\n`;
  });
}
