  ...
  return (
    <Card className="mx-auto w-full max-w-4xl bg-white text-black shadow-lg overflow-hidden rounded-[2rem]">
      <CardContent className="p-12">
        <div className="flex justify-between border-b border-gray-100 pb-8">
          <div>
            {business?.logoUrl && (
              <div className="relative mb-4 h-16 w-32">
                <Image src={business.logoUrl} alt="Logo" fill className="object-contain object-left" />
              </div>
            )}
            <h2 className="text-2xl font-bold text-gray-900">{business?.businessName || "Your Business Name"}</h2>
            <p className="text-gray-500 whitespace-pre-line text-sm mt-1">{business?.address}</p>
          ...
          <div className="text-right">
            ...
            <div className="mt-6 space-y-1 text-sm">
              <p className="font-bold text-gray-900">Invoice #: {invoice.invoiceNumber || "DRAFT"}</p>
              <p className="text-gray-500">Date: {formatDate(invoice.issueDate || new Date())}</p>
              {invoice.dueDate && <p className="text-gray-500">Due Date: {formatDate(invoice.dueDate)}</p>}
              <div className="mt-3">
                <Badge variant="outline" className="uppercase font-bold text-[10px] tracking-widest border-gray-200">
                    {invoice.status}
                </Badge>
              </div>
            </div>
        ...
        <div className="mt-8">
          <h3 className="mb-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">Bill To:</h3>
          <div className="text-sm">
            <p className="font-bold text-gray-900 text-lg">{client?.name || "Client Name"}</p>
            <p className="text-gray-500 whitespace-pre-line mt-1">{client?.address}</p>
            ...
            <TableBody>
              {invoice.items?.map((item, index) => (
                <TableRow key={index} className="border-gray-50 hover:bg-transparent">
                  ...
                  <TableCell className="font-medium text-gray-900">{item.description || "Item description"}</TableCell>
                  <TableCell className="text-center text-gray-900">{item.quantity}</TableCell>
                  <TableCell className="text-right text-gray-900">{formatCurrency(item.unitPrice || 0, currency)}</TableCell>
        ...
        <div className="mt-10 flex justify-end">
          <div className="w-64 space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-gray-900">{formatCurrency(invoice.subtotal || 0, currency)}</span>
            </div>
            ...
            {invoice.taxRate && invoice.taxRate > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-900">{formatCurrency(invoice.taxAmount || 0, currency)}</span>
              </div>
            )}
        ...
        <div className="mt-16 grid grid-cols-2 gap-12 border-t border-gray-100 pt-8 text-sm">
          <div>
            <h4 className="mb-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">Notes</h4>
            <p className="text-gray-600 whitespace-pre-line leading-relaxed">{invoice.customNotes || "No notes."}</p>
          </div>
          <div>
            <h4 className="mb-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">Payment Terms</h4>
