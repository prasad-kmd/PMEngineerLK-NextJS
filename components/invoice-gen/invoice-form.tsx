...
export function InvoiceForm({ clients: initialClients, business, initialData }: InvoiceFormProps) {
  ...
  const handleQuickAddClient = async () => {
    ...
    try {
      ...
      const res = await createClient({ name: newClientName });
      const newClient: Client = { 
        id: res.id, 
        userId: "", // Will be correctly filtered by server anyway
        name: newClientName, 
        email: null, 
        phone: null, 
        address: null, 
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-700">
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
        {/* Invoice Details */}
        <div className="p-8 rounded-[2.5rem] border border-border/40 bg-card/20 backdrop-blur-3xl shadow-sm">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-2">
              <div className="flex items-center justify-between ml-1">
                ...
                <Dialog>
                    <DialogTrigger asChild>
                        <button type="button" className="text-[10px] font-bold text-primary hover:underline flex items-center gap-1 cursor-pointer">
                            <UserPlus className="h-3 w-3" /> Quick Add
                        </button>
                    </DialogTrigger>
                    <DialogContent className="bg-card/90 backdrop-blur-2xl border-border/40 rounded-3xl">
                        <DialogHeader>
                        ...
                        <div className="space-y-4 py-4">
                            <div className="space-y-2">
                                ...
                                <input 
                                    className="w-full h-11 px-4 rounded-xl bg-background/50 border border-border/40 focus:border-primary/50 outline-none"
                                    value={newClientName}
              ...
              <select
                ...
                className="w-full h-11 px-4 rounded-xl bg-background/50 border border-border/40 focus:border-primary/50 focus:ring-4 focus:ring-primary/10 outline-none transition-all appearance-none"
              >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full h-11 px-4 rounded-xl bg-background/50 border border-border/40 focus:border-primary/50 focus:ring-4 focus:ring-primary/10 outline-none transition-all"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground ml-1 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full h-11 px-4 rounded-xl bg-background/50 border border-border/40 focus:border-primary/50 outline-none transition-all"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground ml-1">
            ...
            <div className="pt-6 border-t border-border/40 space-y-3">
              ...
              {(watchedDiscount ?? 0) > 0 && (
                <div className="flex justify-between text-sm text-rose-500">
                  <span>Discount</span>
                  <span>-{formatCurrency(watchedDiscount ?? 0, currency)}</span>
                </div>
              )}
              <div className="flex justify-between items-end pt-2">
                <span className="text-sm font-bold uppercase tracking-widest google-sans">Total</span>
                <span className="text-3xl font-black google-sans tracking-tighter text-primary">
