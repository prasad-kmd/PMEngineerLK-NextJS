    ...
    >
      {({ loading }) => (
        ...
        >
          ...
          {loading ? <span className="hidden sm:inline font-google-sans">Generating...</span> : <span className="hidden sm:inline font-google-sans">{size === "sm" ? "Download" : "Download Resume (PDF)"}</span>}
        </Button>
      )}
    </PDFDownloadLink>
  );
